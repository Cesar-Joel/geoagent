...
for f in AGENTS.md feature_list.json progress/current.md docs/architecture.md docs/conventions.md docs/specs.md docs/verification.md CHECKPOINTS.md pyproject.toml; do
  if [ ! -f "$f" ]; then
...
if [ -d "tests/unit" ]; then
  if command -v make >/dev/null 2>&1 && [ -f "Makefile" ]; then
    if make test-unit 2>&1; then
      ok "make test-unit en verde"
    else
      fail "make test-unit en rojo"
      EXIT_CODE=1
    fi
  else
    if python3 -m pytest tests/unit -q 2>&1; then
      ok "pytest tests/unit en verde"
    else
      fail "pytest tests/unit en rojo"
      EXIT_CODE=1
    fi
  fi
else
  warn "tests/unit/ no existe todavía — la feature de scaffold debe crearlo"
fi

echo ""
echo "── 5. Suites de integración y demo (informativas) ──────"

if [ -d "tests/integration" ]; then
  warn "Existe tests/integration/. Ejecuta 'make test-integration' cuando la feature lo requiera."
else
  warn "tests/integration/ no existe todavía."
fi

if [ -f "tools/verify_e2e.py" ]; then
  warn "Existe tools/verify_e2e.py. Ejecuta 'make demo' antes de cerrar features de flujo end-to-end."
fi
...
if [ $EXIT_CODE -eq 0 ]; then
  ok "Entorno listo. Puedes empezar a trabajar o cerrar la sesión."
else
